...
class Metric(nn.Module):
    def __init__(self, class_num=655, backbone='resnet101', pretrained_backbone=True, use_feats=(2,4,), fc_dims=(512,)):
        ...
        self.final_linear = nn.Linear(class_num, 256) # final class num
    def forward(self, x):
        img_feats = self.img_encoder(x)
        pred_scores = self.fc_layers(img_feats)
        projected_scores = self.final_linear(pred_scores)
